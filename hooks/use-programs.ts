...
export function usePrograms() {
  ...
  const [university, setUniversity] = useQueryState("university", { defaultValue: "all" })
  const [season, setSeason] = useQueryState("season", { defaultValue: "all" })
  const [month, setMonth] = useQueryState("month", parseAsInteger.withDefault(0))
  ...
  useEffect(() => {
    setTempFilters({
      ...
      month: month as FilterState["month"],
    })
  ...
  const applyFilters = useCallback(() => {
    ...
    setCurrentPage(1)
  }, [tempFilters, setPriceRange, setType, setUniversity, setSeason, setMonth, setCurrentPage])
