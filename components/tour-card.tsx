...
export const TourCard = ({ tour, viewMode }: TourCardProps) => {
  return (
    ...
    >
      <div className={cn("relative", viewMode === "list" ? "md:w-1/3" : "w-full")}>
        <Image
          src={tour.image}
          alt={tour.title}
          width={200}
      ...
      <div className={cn("flex flex-1 flex-col", viewMode === "list" && "md:max-w-2xl")}>
        <CardHeader>
          ...
          <CardDescription className="line-clamp-2 leading-normal">{tour.description}</CardDescription>
        </CardHeader>

        <CardFooter className={cn("mt-auto space-x-2", viewMode === "list" && "md:justify-end")}>
          <Button asChild>
