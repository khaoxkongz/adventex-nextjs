...
const TourSearch = React.forwardRef<HTMLDivElement, TourSearchProps>(({ className, children, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
...
const TourSearchHeader = React.forwardRef<HTMLDivElement, TourSearchHeaderProps>(
  ({ className, totalCount, ...props }, ref) => {
    return (
      <div ref={ref} className={cn("flex flex-col gap-2", className)} {...props}>
        <h2 className="text-2xl font-semibold">ค้นหาแพ็คเกจ</h2>
        <p className="text-sm text-muted-foreground">
          พบทั้งหมด <span className="font-medium text-orange-500">{totalCount}</span> รายการ
        </p>
      </div>
    )
  }
...
const TourSearchControls = React.forwardRef<HTMLDivElement, TourSearchControlsProps>(
  ({ className, searchQuery, onSearchChange, viewMode, onViewModeChange, ...props }, ref) => {
    return (
      <div ref={ref} className={cn("flex w-full items-center gap-4 md:w-auto", className)} {...props}>
